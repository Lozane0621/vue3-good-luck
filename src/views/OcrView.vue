    ...
    <div class="flex-auto">
      <section class="min-h-[100vh]">
        ...
        <div class="text-center pt-5">
          <h1
            class="from-indigo-400 to-pink-700 bg-gradient-to-r bg-clip-text text-2xl font-extrabold uppercase text-transparent"
          >
            {{ appName }}
          </h1>
        ...
        <div class="grid mt-12 place-items-center gap-6">
          ...
          <form
            class="max-w-prose w-full space-y-8"
          ...
          >
            ...
            <template v-if="!hasImage">
              <div
                class="flex justify-center border-2 border-gray-600 rounded-md border-dashed px-10 pb-10 pt-9"
                  ...
                  <div class="flex">
                    ...
                    >
                      ...
                      >
                        <div
                          aria-hidden="true"
                          class="i-carbon-chevron-down h-5 w-5 text-gray-400"
                        />
                      </span>
                    </ListboxButton>
                  </div>
            ...
            <div v-if="hasResults" class="space-y-6">
              <div class="flex justify-around">
                <h3 class="text-lg font-semibold">识别结果</h3>

                <span
                  class="inline-flex items-center rounded-full bg-indigo-800 px-2.5 py-0.5 text-xs font-medium"
                >
                  <i
                    :class="{
              ...
              <div class="relative">
                ...
                >
                  <div
                    :class="[copied ? 'i-carbon-task' : 'i-carbon-task-add']"
                    aria-hidden="true"
                    class="h-6 w-6"
                  />

                  <span class="sr-only">复制输出</span>
                </button>
              </div>
            </div>

            <!-- Buttons -->
            <div v-if="hasImage && !isProcessing" class="text-center pb-5">
              <button
                v-if="!hasResults"
...
import { appDescription, appName, supportedLanguages, supportedMimeTypes } from '@/constants'
...
function ocrWorkerLogger(message) {
...
function onClipboardPaste(event) {
  ...
  if (isProcessing.value || hasResults.value) return
...
async function startRecognition() {
  ...
  } = await ocrWorker.recognize(media.value, {
    rotateAuto: true
  })
